...
public class FriendRequestManager implements FriendRequestService {
    ...
    @Override
    public void sendFriendRequest(String toUserEmail, User currentUser) throws ResourceNotFoundException, IllegalArgumentException {
        ...
        if (toUser == null) {
            log.warn(String.format("Failed to send friend request from user %d to user with email %s. User not found.", currentUser.getId(), toUserEmail));
            throw new ResourceNotFoundException("User not found");
        }

        if (toUser.getUserProfile() == null) {
            log.warn(String.format("Failed to send friend request from user %d to user %d. No profile created.", currentUser.getId(), toUser.getId()));
        ...
        if (!friendsRepository.getByFromUserIdAndToUserId(currentUser.getId(), toUser.getId()).isEmpty()) {
            log.info(String.format("Ignoring friend request from user %d to user %d. Request already in place.", currentUser.getId(), toUser.getId()));
            return;
        }
    ...
    public void acceptFriendRequest(long id, User currentUser) {
        friendsRepository.findById(id).ifPresent(friendRequest -> {
            if (!friendRequest.getTo().equals(currentUser.getUserProfile())) {
                log.error(String.format("Friend request from %d to %d was accepted by %d, but profile id's did not match!",
    ...
    public void deleteFriendRequest(long id, User currentUser) {
        friendsRepository.findById(id).ifPresent(friendRequest -> {
            if (!friendRequest.getTo().equals(currentUser.getUserProfile())) {
                log.error(String.format("Friend request from %d to %d was denied by %d, but profile id's did not match!",
                        friendRequest.getFrom().getUser().getId(),
                        friendRequest.getTo().getUser().getId(),
                        currentUser.getId()));
                throw new BadCredentialsException("");
